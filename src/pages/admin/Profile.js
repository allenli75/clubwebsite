...
import { updateProfile, uploadLogo } from '../../redux/actions/profile';
...
const Profile = ({
  ...
  const [recrStartDate, setRecrStartDate] = useState(
    profile.recruiting_start == null
      ? null
      : getDateOnly(profile.recruiting_start)
  );
  const [recrEndDate, setRecrEndDate] = useState(
    profile.recruiting_end == null ? null : getDateOnly(profile.recruiting_end)
  );
  const [appStartDate, setAppStartDate] = useState(
    profile.apply_deadline_start == null
      ? null
      : getDateOnly(profile.apply_deadline_start)
  );
  const [appEndDate, setAppEndDate] = useState(
    profile.apply_deadline_end == null
      ? null
    ...
    if (!checkDateError()) {
      const newProfile = {
        ...
        apply_deadline_start:
          recruiting.value === 1 && appReq.value === 1
      ...
      try {
        ...
        await Promise.all([uploadLogoPic(logoImage)]);
  ...
  return (
    <div>
      ...
      <div className="admin-text">
        Add an organization logo, edit your tags, membership status, recruitment
        period, or application requirements.
      </div>
      ...
      <div
          ...
          <div className="formGroup">
            ...
            {recruiting.value === 1 && (
              <div>
                ...
                <p className="subtitle" style={{ float: 'right' }}>
                  <span style={{ color: '#FF0000' }}>*</span> Your recruitment
                  status will automatically update depending on these dates.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="modal-right">
          ...
          <p className="subtitle">
            <span style={{ color: '#FF0000', marginLeft: '40px' }}>*</span>{' '}
            Please make sure your logo is at least 360 x 360 pixels. <br></br>
            <a
              href="https://www.photoresizer.com/"
              target="_blank"
              rel="noopener noreferrer">
              <span style={{ color: '#54a0f1', marginLeft: '40px' }}>
                Click here
              </span>
            </a>{' '}
