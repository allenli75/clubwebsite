...
const GalleryUpload = ({
  ...
  const [gallery, setGallery] = useState(
    profile.gallery_media
      ? profile.gallery_media.map((el) => ({ ...el, changed: false }))
      : []
  );
  useEffect(() => {
  ...
  return (
    <div className="gallery-upload-wrapper">
          ...
          gallery.map((el, ind) => (
            <div className="image-card">
              <div>
                ...
                <p className="subtext">
                  Upload image max 2MB. Image ratio will be cropped to 16 : 9
                </p>
              </div>
              <div className="bottom-section">
                ...
                <input
                  ...
                  maxLength="50"
                  value={el.caption}
                  onChange={(e) => updateCaption(e.target.value, ind)}
                  placeholder="Caption for photo"></input>
      ...
      <p>
        <span style={{ color: '#FF0000', borderBottom: '10px' }}>*</span> This
        feature is undergoing beta testing.
      </p>
    </div>
  );
};
